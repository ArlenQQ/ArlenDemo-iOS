...
- (void)viewDidLoad {
    ...
    @WeakObj(self);
    [self.webView addObserverBlockForKeyPath:ASKeyPath(selfWeak.webView, estimatedProgress) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
        
        selfWeak.progressView.progress = selfWeak.webView.estimatedProgress;
        // 加载完成
        if (selfWeak.webView.estimatedProgress  >= 1.0f ) {
            
            [UIView animateWithDuration:0.25f animations:^{
                selfWeak.progressView.alpha = 0.0f;
                selfWeak.progressView.progress = 0.0f;
            }];
            
        }else{
            selfWeak.progressView.alpha = 1.0f;
        }
        
    }];
    
    [self.webView addObserverBlockForKeyPath:ASKeyPath(self.webView, title) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
        
        if (!ASIsEmpty(newVal) && [newVal isKindOfClass:[NSString class]] && [self webViewController:self webViewIsNeedAutoTitle:self.webView]) {
    ...
    [self.webView.scrollView addObserverBlockForKeyPath:ASKeyPath(self.webView.scrollView, contentSize) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
        [selfWeak webView:selfWeak.webView scrollView:selfWeak.webView.scrollView contentSize:selfWeak.webView.scrollView.contentSize];
    }];
    
    if (self.gotoURL.length > 0) {
        [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.gotoURL]]];
    }else if (!ASIsEmpty(self.contentHTML)) {
...
- (UIView *)asNavigationBarLeftView:(ASNavigationBar *)navigationBar
{
    ...
    leftView.backgroundColor = kColorName(kCX_NAVTHEME);//[UIColor yellowColor];
    
    self.backBtn.as_origin = CGPointZero;
    
    self.closeBtn.as_x = leftView.as_width - self.closeBtn.as_width;
...
#pragma mark - ASWebViewControllerDataSource
// 默认需要, 是否需要进度条
...
//        [config.userContentController addScriptMessageHandler:selfWeak name:<#(nonnull NSString *)#>];
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.as_height = 1;
        
        progressView.as_width = kScreenWidth;
        
        progressView.as_y = self.as_navgationBar.as_height;
...
- (UIButton *)backBtn
{
    if(_backBtn == nil)
    {
        ...
        [btn setImage:[UIImage imageNamed:@"navigationbar_back_white"] forState:UIControlStateNormal];
        
        //[btn setImage:[UIImage imageNamed:@"navigationButtonReturnClick"] forState:UIControlStateHighlighted];
        
        btn.as_size = CGSizeMake(44, 44);
...
- (void)dealloc
{
    NSLog(@"ASWebViewController -- dealloc");
