...
#pragma mark - ASTextViewControllerDelegate
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (textField.isEmptyAutoEnable && (ASIsEmpty(textField.text.stringByTrim) || ASIsEmpty(current))) {
        if (ASIsEmpty(current)) {
...
- (void)checkIsEmpty:(BOOL)isEmpty textField:(UITextField *)textField {
    if (ASIsEmpty(self.requiredTextFields)) {
        return;
    }
    
    if ([self respondsToSelector:@selector(textViewControllerRelationButtons:)]) {
        if (ASIsEmpty([self textViewControllerRelationButtons:self])) {
            return;
        }
    }else
    {
    ...
    if (!isEmpty) {
        [self.requiredTextFields enumerateObjectsUsingBlock:^(UITextField *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj != textField && ASIsEmpty(obj.text.stringByTrim)) {
                isButtonEnabled = NO;
                *stop = YES;
            }
        }];
...
#pragma mark - ASTextViewControllerDataSource
...
#pragma mark - ASTextViewControllerTextField

static void *isEmptyAutoEnableKey = &isEmptyAutoEnableKey;
@implementation UITextField (ASTextViewController)
...
@implementation ASTextViewControllerTextField
- (NSString *)text {
