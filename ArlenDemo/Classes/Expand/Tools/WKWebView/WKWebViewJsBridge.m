...
- (void)registerHandler:(NSString *)handleName handle:(void(^)(id data, void(^)(id responseData)))handle {
    if (handle && !ASIsEmpty(handleName)) {
...
- (instancetype)initWithWebView:(WKWebView *)webView delegate:(id<WKNavigationDelegate>)delegate {
    if (self = [self init]) {
        ...
        @WeakObj(webView);
        @WeakObj(delegate);
        _webView = webViewWeak;
        _webView.navigationDelegate = selfWeak;
        _delegate = delegateWeak;
        
        [WKBridgeTool bridgeRegisterDefault:selfWeak];
    }
    return self;
}
